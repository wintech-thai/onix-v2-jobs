...
def get_webhook_config(conn, merchantId, eventName = 'Payment.Success')
...
def process_payment_success_job(stream, data, conn, eventName = 'Payment.Success')
  ...
  whc = get_webhook_config(conn, merchantId, eventName)
  if (whc.nil?)
    ...
    entries.each do |stream, messages|
      messages.each do |id, fields|
        ...
        if ['Payment.Success', 'PaymentOut.Success'].include?(jobType)
          process_payment_success_job(stream, data, conn, jobType)
        end
